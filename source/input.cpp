...
void application::continuousKeyInput(double deltaTime) {
    ...
    if (keyboardState[SDL_SCANCODE_KP_DIVIDE] == SDL_PRESSED) {
        this->myHelicopter->increaseThrottle(-deltaTime*20.0f);
    }
    ...
    if (keyboardState[SDL_SCANCODE_KP_7] == SDL_PRESSED) {
        this->myHelicopter->bankLeft(-deltaTime);
    }
    if (keyboardState[SDL_SCANCODE_KP_9] == SDL_PRESSED) {
        this->myHelicopter->bankLeft(deltaTime);
    }
